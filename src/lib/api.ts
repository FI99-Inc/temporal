...
export interface PersonalView { view: Snapshot; trace: TraceInfo }
...
async function request<T>(command: string, args: Record<string, unknown> = {}): Promise<T> {
  ...
  const response = command === 'import_trace_json'
    ? await fetch(`/__temporal/${command}`, { method:'POST', headers:{'Content-Type':'application/json'}, body:String(args.json) })
