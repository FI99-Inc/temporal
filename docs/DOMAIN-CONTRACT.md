...
Those target requirements are creation-time eligibility rules. A retained prior Suggestion is structurally validated (well-formed key/times/kind, typed target reference, and retained historical reason references) even if its target is now Done, removed, resolved, or otherwise ineligible. Current target records/tombstones must still resolve its typed reference; historical Window/pressure keys retain their producing EvaluationKey rather than needing to exist in current output. Current eligibility is checked by validity evaluation, after the basis-change and expiry checks, not by rejecting the historical record during snapshot validation. No historical eligibility claim is reconstructed from newer source state.
...
The fixture envelope combines the revisioned factual/user snapshot, evaluation request, and separately retained prior_suggestions. Supplying previously produced suggestions for a validity check does not itself change snapshot_revision or promote them to current facts. A change to factual/user inputs or settings still changes the revision/basis as specified in Section 3.
